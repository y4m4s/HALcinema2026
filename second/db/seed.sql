...
INSERT INTO movies (
    ...
    rating, duration_min, poster_url, tagline, synopsis, genre, screening_start_date
) VALUES
...
INSERT INTO reservations
    (id, schedule_id, member_id, customer_name, customer_name_kana,
     customer_email, customer_tel, status, created_at, updated_at)
SELECT
    schedule_id,
    schedule_id, NULL, 'シード予約', 'しーどよやく',
    'seed-reservation@example.com', '00000000000',
    'confirmed', '2026-05-20T10:00:00+09:00', '2026-05-20T10:00:00+09:00'
FROM _schedule_fill;

INSERT INTO reservation_details (id, reservation_id, ticket_type_id, quantity, unit_price, subtotal)
WITH target AS (
    SELECT
        f.schedule_id,
        CASE f.status
            WHEN 'soldout' THEN sc.capacity
            WHEN 'few'     THEN sc.capacity - CAST(sc.capacity * 0.08 AS INTEGER)
            ELSE                CAST(sc.capacity * 0.4 AS INTEGER)
        END AS reserve_count
    FROM _schedule_fill f
    JOIN schedules sch ON sch.id = f.schedule_id
    JOIN (
        SELECT screen_id, COUNT(*) AS capacity
          FROM seats
         WHERE is_active = 1
         GROUP BY screen_id
    ) sc ON sc.screen_id = sch.screen_id
)
SELECT
    schedule_id,
    schedule_id,
...
WITH target AS (
    ...
    JOIN (
        SELECT screen_id, COUNT(*) AS capacity
         ...
         WHERE is_active = 1
         GROUP BY screen_id
    ) st ON st.screen_id = sch.screen_id
),
ranked AS (
...
('Q001', 'はる太郎', 'hal-taro@example.com', '上映情報',
 ...
 '2026-06-04T00:00:01+09:00', 'resolved',
 '翌月分のスケジュールは毎月20日前後に公開予定です。',
 '2026-06-05T00:00:01+09:00', 'はる太郎2');
